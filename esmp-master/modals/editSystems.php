<div class="header"><i class="desktop icon"></i>Editar Sistema</div>
<div class="content">
    <form method="POST" action="javascript:void(0);" id="formAlterSystem">
        <div class="field">
...
<div class="actions">
    ...
    <div id="" class="ui positive right labeled icon button btnCadastrarS" onclick="alterarSistemas()">
        Cadastrar
        <i class="play icon"></i>
    </div>
</div>
...
<script>
    ...
    function alterarSistemas() {
        var formData = new FormData($('#formAlterSystem')[0]);
        var idSistema = $(".btnCadastrarS").attr('id');
        var imagem = $('#file').prop('files');
        formData.append('action', 'toAlterSystem');
        formData.append('id', idSistema);

        if(imagem != undefined){
            formData.append('image', $('#file').prop('files')[0]);
        }
        else{
            formData.append('imagePath', $(".upImageSystem").attr("src"));
        }
    ...
    $('.addImageSystem').on("click", function(){
        $("#formAlterSystem").append($('<input />', {type: "file", id: 'file', name: 'file' }).change(verificaMostraBotao));
        $("#file").trigger('click');
    });
</script>